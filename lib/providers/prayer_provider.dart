...
class PrayerProvider extends ChangeNotifier {
  ...
  callNotification() {
    ...
    LocalNotfiService().addNamazNotification(
        20, "Fajar Prayer Time", "🕌 Fajar prayer time in Karachi", DateTime.now().millisecondsSinceEpoch + 2000,
        channelID: 'namaz', channelName: "namaz");
  }
  ...
  initPrayerTiming(BuildContext context) {
    apiService.callGetApi("http://ip-api.com/json").then((value) {
      ...
      apiService
          .callGetApi('$prayerTimeApiUrl${city['city']}&country=${city['country']}&method=1&school=1')
          .then((namaz) {
        print(namaz.body);
  ...
  addMedicineTimeNotifiction() {
    ...
    // 30 night
    LocalNotfiService().addNamazNotification(
        10, "Morning Medicine Reminder", "Medicine Time is up", getNotificationDate("9:30", 1),
  ...
  setSchedulePrayerNotification(int prayer, String time, String city) {
    switch (prayer) {
      case 0:
        ...
        // for(var i = 1; i < 8; i++){
        LocalNotfiService().addNamazNotification(
            01, "Fajar Prayer Time", "🕌 Fajar prayer time in $city", getNotificationDate(time, 1),
      ...
      case 1:
        // for(var i = 1; i < 8; i++){
        LocalNotfiService().addNamazNotification(
            11, "Dhuhr Prayer Time", "🕌 Dhuhr prayer time in $city", getNotificationDate(time, 1),
      ...
      case 2:
        // for(var i = 1; i < 8; i++){
        LocalNotfiService().addNamazNotification(
            22, "Asar Prayer Time", "🕌 Asar prayer time in $city", getNotificationDate(time, 1),
      ...
      case 3:
        // for(var i = 1; i < 8; i++){
        LocalNotfiService().addNamazNotification(
            33, "Maghrib Prayer Time", "🕌 Maghrib prayer time in $city", getNotificationDate(time, 1),
      ...
      case 4:
        // for(var i = 1; i < 8; i++){
        LocalNotfiService().addNamazNotification(
            44, "Isha Prayer Time", "🕌 Isha prayer time in $city", getNotificationDate(time, 1),
