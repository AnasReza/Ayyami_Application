...
class PrayerProvider extends ChangeNotifier {
  ...
  List<String> medicineTimingList = ["Morning", "Evening", "Night"];
  List<String> medicineTimingListUrdu = ["صبح", "شام", "رات"];
  ...
  String gorgeonTodayDateFormated =
      _todayDateFormated.format(_gorgeonDateToday);
  ...
  callNotification() {
    ...
    LocalNotfiService().addNamazNotification(
        20,
        "Fajar Prayer Time",
        "🕌 Fajar prayer time in Karachi",
  ...
  initPrayerTiming(BuildContext context) {
    apiService.callGetApi("http://ip-api.com/json").then((value) {
      ...
      apiService
          .callGetApi(
              '$prayerTimeApiUrl${city['city']}&country=${city['country']}&method=1&school=1')
          .then((namaz) {
        print(namaz.body);
  ...
  addMedicineTimeNotifiction() {
    ...
    // 20 evening
    // 30 night
    LocalNotfiService().addNamazNotification(10, "Morning Medicine Reminder",
        "Medicine Time is up", getNotificationDate("9:30", 1),
        channelID: "10", channelName: 'Morning Medicine');
  }
  ...
  setSchedulePrayerNotification(int prayer, String time, String city) {
    switch (prayer) {
      case 0:
        // fajar prayer
        // for(var i = 1; i < 8; i++){
        LocalNotfiService().addNamazNotification(01, "Fajar Prayer Time",
            "🕌 Fajar prayer time in $city", getNotificationDate(time, 1),
            channelID: 'fajr01', channelName: 'fajr');
        // }
        break;
      // duhar prayer
      case 1:
        // for(var i = 1; i < 8; i++){
        LocalNotfiService().addNamazNotification(11, "Dhuhr Prayer Time",
            "🕌 Dhuhr prayer time in $city", getNotificationDate(time, 1),
            channelID: 'Dhuhr11', channelName: 'Dhuhr');
        // }
        break;
      // asar prayer
      case 2:
        // for(var i = 1; i < 8; i++){
        LocalNotfiService().addNamazNotification(22, "Asar Prayer Time",
            "🕌 Asar prayer time in $city", getNotificationDate(time, 1),
            channelID: 'Asar22', channelName: 'Asar');
        // }
        break;
      // maghrib prayer
      case 3:
        // for(var i = 1; i < 8; i++){
        LocalNotfiService().addNamazNotification(33, "Maghrib Prayer Time",
            "🕌 Maghrib prayer time in $city", getNotificationDate(time, 1),
            channelID: 'Maghrib33', channelName: 'Maghrib');
        // }
        break;
      // isha prayer
      case 4:
        // for(var i = 1; i < 8; i++){
        LocalNotfiService().addNamazNotification(44, "Isha Prayer Time",
            "🕌 Isha prayer time in $city", getNotificationDate(time, 1),
            channelID: 'isha44', channelName: 'isha');
        // }
        break;
    }
  }
