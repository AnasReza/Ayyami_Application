...
class TimerBox extends StatefulWidget {
  final Function(bool mensis, String regulationMessage) mensis;
  final String islamicMonth;

  const TimerBox({Key? key, required this.mensis, required this.islamicMonth})
      : super(key: key);
...
class _TimerBoxState extends State<TimerBox> {
              ...
              child: Row(
                ...
                children: [
                  ...
                  Column(
                    children: [
                      AppText(
                        text: isMensesStart ? pro.getDays.toString() : '00',
                  ...
                  Column(
                    children: [
                      AppText(
                        text: isMensesStart ? pro.getHours.toString() : '00',
                        color: AppColors.pink,
                        fontSize: 56.722694396972656.sp,
                        fontWeight: FontWeight.w700,
                      ),
                  ...
                  Column(
                    children: [
                      AppText(
                        text: isMensesStart ? pro.getmin.toString() : '00',
                        color: AppColors.pink,
                        fontSize: 56.722694396972656.sp,
                        fontWeight: FontWeight.w700,
                      ),
                  ...
                  Column(
                    children: [
                      AppText(
                        text: isMensesStart ? pro.getSec.toString() : '00',
                        color: AppColors.pink,
                        fontSize: 56.722694396972656.sp,
                        fontWeight: FontWeight.w700,
                      ),
          ...
          Positioned(
            ...
            child: GestureDetector(
              onTap: () {
                ...
                print(tuhurDays);
                print(tuhurProvider.getTuhurID);
                // print(tuhurProvider.getStartTime.toDate());
                print(tuhurProvider.getFrom);
                if (!isMensesStart) {
                  if (isTuhurStart) {
                    if (tuhurFrom == 0) {
                      showStartDialog(text);
                    } else {
                      //if last tuhur was from postnatal end or tuhurfrom = 1
                      //if postnatal < 40 or = 40 and tuhur > 15 = Menses can be started
                      //Menses cant be started if postnatal has not passed by 15 days
                      if (userProvider.getMarried == 'Married' &&
                          userProvider.getPostNatalData.isNotEmpty) {
                        print(tuhurDays);
                        if (tuhurDays < 0) {}
                        if (tuhurDays >= 15) {
                          // e.g Pregnancy 20L 17T
                          showStartDialog(text);
                        } else {
                          // e.g Pregnancy 40L 4T
                          if (userProvider.getPostNatalData.first.get('days') >=
                              40) {
                            //user migt have to put another date
                            showStartDialog(text);
                            // toast_notification().toast_message(
                            //     text['post_natal_days_completed']!);
                          } else {
                            // e.g Pregnancy 20L 4T
                            toast_notification().toast_message(
                                text['should_post-natal_again']!);
                          }
                        }
                      } else {
                        //if from = 1 than post natal must be there
                        toast_notification()
                            .toast_message(text['should_post-natal_again']!);
                      }
                    }
                  } else if (userProvider.getArePregnant) {
                    toast_notification()
                        .toast_message(text['should_stop_pregnancy']!);
                  } else if (!isTuhurStart) {
                    toast_notification()
                        .toast_message(text['should_tuhur_start']!);
                  }
                } else {
                  showStopDialog(text);
                }
  ...
  //   //it is not necessary that last menses must be valid e.g 6Feb tuhur starts and 8Feb pregnancy starts tuhur ends
  //   //hence 2 days is not valid duration to be added to last menses in case of married
  //   //possible solution introduce habit ids
  ...
  //     print("mtp: " + mensesTimeMap.toString());
  ...
  //     print("td:" + tuhurDuration.toString());
          ...
          return DialogDateTime(
            getDateTime: (date, time) {
              ...
              mensesTrack.startMensisTimer(mensesProvider, uid, tuhurProvider,
                  Timestamp.fromDate(startDate), false);
        ...
        builder: (dialogContext) {
          return DialogDateTime(
            getDateTime: (date, time) {
              ...
              print("3: ${habitProvider.habitModel.toTuhurDurationMap()}");
              String message = MensesTracker().processStopMensesTimer(
                  mensesProvider,
                  tuhurProvider,
                  habitProvider,
                  uid, 
