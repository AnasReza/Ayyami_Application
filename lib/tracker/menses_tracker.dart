...
class MensesTracker {
  ...
  void startMensisTimer(
      MensesProvider mensesProvider, String uid, TuhurProvider tuhurProvider, Timestamp startTime) {
    // var startTime = Timestamp.now();
    var startDate = startTime.toDate();
    var now = DateTime.now();
    _stopWatch = StopWatchTimer(
        mode: StopWatchMode.countUp, presetMillisecond: now.difference(startDate).inMilliseconds);
    var timeMap =
        Utils.timeConverter(Duration(milliseconds: now.difference(startDate).inMilliseconds));
  ...
  void startMensisTimerFromPregnancy(
      MensesProvider mensesProvider, String uid, Timestamp startTime) {
    // var startTime = Timestamp.now();
    var startDate = startTime.toDate();
    var now = DateTime.now();
    _stopWatch = StopWatchTimer(
  ...
  void startMensisTimerWithTime(
      MensesProvider mensesProvider, String uid, int milliseconds, Timestamp startTime) {
  ...
  String stopMensesTimer(MensesProvider mensesProvider, TuhurProvider tuhurProvider, String uid,
    ...
    try {
      ...
      if (currentMensesStartDay.isAtSameMomentAs(assumptionStart!) &&
        ...
        if (days < 3) {
          ...
          if (beginner == 'Beginner') {
            if (islamicMonth.contains('Rama')) {
              regulationMessage =
                  'beginner_before_3_days'.tr + 'beginner_before_3_days_ramadhan'.tr;
            } else {
            ...
            if (married == 'Married') {
              if (islamicMonth.contains('Rama')) {
                ...
                regulationMessage =
                    'accustomed_before_3_days'.tr + 'accustomed_before_3_days_married'.tr;
              }
            } else {
              if (islamicMonth.contains('Rama')) {
                regulationMessage =
                    'accustomed_before_3_days'.tr + 'accustomed_before_3_days_ramadhan'.tr;
              } else {
                regulationMessage = 'accustomed_before_3_days'.tr;
              }
            }
          ...
          if (days < 10) {
            uploadMensesEndTime(Timestamp.fromDate(currentMensesEndDay), mensesID, days, hours,
                minutes, seconds, mensesProvider, tuhurProvider, userProvider.getUid!, false);
            String? beginner = userProvider.getBeginner;
            if (beginner == 'Beginner') {
              if (islamicMonth.contains('Rama')) {
                regulationMessage = text['beginner_after_3_days_before_10_days']! +
            ...
            if (days == 10) {
              ...
              String? beginner = userProvider.getBeginner;
              if (beginner == 'Beginner') {
                uploadMensesEndTime(Timestamp.fromDate(currentMensesEndDay), mensesID, 10, 0, 0, 0,
                    mensesProvider, tuhurProvider, uid, true);
              } else {
                var lastMap = userProvider.getLastMensesTime;
                int days = lastMap['day']!;
                int hours = lastMap['hours']!;
                int minutes = lastMap['minutes']!;
                int seconds = lastMap['seconds']!;
                var endDate = currentMensesStartDay
                ...
                uploadMensesEndTime(Timestamp.fromDate(endDate), mensesID, dayEnd, hourEnd,
                    minuteEnd, secondEnd, mensesProvider, tuhurProvider, uid, true);
              }
            }
          }
          String? beginner = userProvider.getBeginner;
        ...
        if (days < 3) {
          ...
          if (beginner == 'Beginner') {
            if (islamicMonth.contains('Rama')) {
              regulationMessage =
                  text['beginner_before_3_days']! + text['beginner_before_3_days_ramadhan']!;
            } else {
            ...
            if (married == 'Married') {
              if (islamicMonth.contains('Rama')) {
                ...
                regulationMessage =
                    text['accustomed_before_3_days']! + text['accustomed_before_3_days_married']!;
              }
            } else {
              if (islamicMonth.contains('Rama')) {
                regulationMessage =
                    text['accustomed_before_3_days']! + text['accustomed_before_3_days_ramadhan']!;
              } else {
                regulationMessage = text['accustomed_before_3_days']!;
              }
            }
          ...
          if (days < 10) {
            uploadMensesEndTime(Timestamp.fromDate(assumptionEnd), mensesID, days, hours, minutes,
                seconds, mensesProvider, tuhurProvider, userProvider.getUid!, false);
            String? beginner = userProvider.getBeginner;
            if (beginner == 'Beginner') {
              if (islamicMonth.contains('Rama')) {
                regulationMessage = text['beginner_after_3_days_before_10_days']! +
            ...
            if (days == 10) {
              ...
              uploadMensesEndTime(Timestamp.fromDate(assumptionEnd), mensesID, 10, 0, 0, 0,
                  mensesProvider, tuhurProvider, uid, true);
              String? beginner = userProvider.getBeginner;
              if (beginner == 'Beginner') {
                uploadMensesEndTime(Timestamp.fromDate(assumptionEnd), mensesID, 10, 0, 0, 0,
          ...
          if (days < 10) {
            ...
            if (days == 10) {
              ...
              if (beginner == 'Beginner') {
                ...
                int minutes = lastMap['minutes']!;
                int seconds = lastMap['seconds']!;
                var endDate = assumptionStart
                    .add(Duration(days: days, hours: hours, minutes: minutes, seconds: seconds));

                var diff = endDate.difference(assumptionStart);
                var map = Utils.timeConverter(diff);
                int dayEnd = map['days']!;
                int hourEnd = map['hours']!;
                int minuteEnd = map['minutes']!;
            ...
            if (beginner == 'Beginner') {
              if (islamicMonth.contains('Rama')) {
                regulationMessage = 'beginner_after_3_days_before_10_days'.tr +
                    'beginner_after_3_days_before_10_days_ramadhan'.tr;
              } else {
                regulationMessage = 'beginner_after_3_days_before_10_days'.tr;
              }
            } else {
      ...
      if (days! > 0 && days < 10) {
        print('10@! e');
        uploadMensesEndTime(currentEnd, mensesID, days, hours!, minutes!, seconds!, mensesProvider,
        ...
        print('12@! e');
        uploadMensesEndTime(currentEnd, mensesID, 10, 0, 0, 0, mensesProvider, tuhurProvider,
            userProvider.getUid!, true);
        String? beginner = userProvider.getBeginner;
        if (beginner == 'Beginner') {
          if (islamicMonth.contains('Rama')) {
            regulationMessage =
  ...
  void stopTimerWithDeletion(
      String mensesID, MensesProvider mensesProvider, TuhurProvider tuhurProvider) {
  ...
  Map<String, DateTime> assumptionOfMenses(
      UserProvider provider, DateTime startTime, DateTime endTime) {
